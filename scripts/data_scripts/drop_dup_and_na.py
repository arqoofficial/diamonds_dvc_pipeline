...
def process_data_pandas(f_input, f_output):
    ...
    if nan_amount == 0:
        ...
        logger.warning(
            f"There is/are {nan_amount} NaN(s), they will be dropped"
        )
    ...
    logger.success(
        "Prepared diamonds.csv without NaN and duplicates "
        f"was saved as `{f_output}`"
    )
...
def main():
    ...
    if not Path(f_input).exists():
        logger.error(
            f"diamonds csv not found, please check path correctnes: {f_input}"
        )
        raise SystemExit
